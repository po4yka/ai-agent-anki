...
- Works from command line but not from apps
...
- Card creates but missing content
...
**Symptoms:**
- `updateNoteFields` returns success but card unchanged
...
**Symptoms:**
- Cards appearing too frequently or rarely
- Intervals not matching expectations
...
**Symptoms:**
- Existing cards behave strangely after enabling FSRS

**Solutions:**

- **Don't reset:** Existing intervals will adapt
- Cards will gradually adjust over next few reviews
- FSRS reads your history and adapts
...
1. **Limit search results:**
   ...
   ```
   added:30 deck:MyDeck
   ```

2. **Optimize database:**
   Tools > Check Database

3. **Close other add-ons:**
   Some add-ons slow browser significantly
...
1. **Increase timeout:**
   Set `ANKI_CONNECT_TIMEOUT` environment variable (ms)
...
3. **MCP Server Issues:** https://github.com/ankimcp/anki-mcp-server/issues
4. **Reddit:** r/Anki for community help
